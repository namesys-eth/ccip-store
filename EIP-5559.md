...
For any storage other than a blockchain with a wallet extension, the client must call the `gatewayUrl` via a `POST` request. The structure of the `POST` is protocol-specific and left up to individual protocols to handle internally. Besides the `POST` request, `username` and `dataType` are two other protocol-specific parameters that we have encountered in the text before. These three parameters should be defined by the protocols through a native improvement proposal. For example, `POST` format, `username` and `dataType` for ENS are described in ENSIP-X.
...
function setAddr(
    ...
    bytes memory metadata = abi.encode(
        ...
        metadataUrl // GraphQL metadata endpoint (required by ENSIP-16)
    )
