...
(
    ...
    string metadataUrl // API endpoint for GET-ing sequence number
) = getMetadata(node);
...
where `dataType` parameters are protocol-specific; they are defined in ENSIP-6 and ENSIP-9 for ENS, e.g. `text/avatar`, `address/60` etc.
...
function setAddr(
    ...
    bytes memory metadata = abi.encode(
        ...
        metadataUrl // GraphQL endpoint for sequence counter (per ENSIP-16)
    )
