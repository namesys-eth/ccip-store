...
(
    ...
    string metadataUrl // API endpoint for GET-ing encoded revision
) = getMetadata(node);
...
In this example, the client should process the metadata according to the following specifications. The client should request the user for an IPNS signature verifiable against the signer's CID returned in the metadata. If verified, the client then additionally requires a `revision` value encoding the IPNS record version and validity duration to make the IPNS update. This counter should be provided by the storage provider through an API endpoint along with any other optional metadata; the interface for this API is left up to individual implementations and/or protocols to define (e.g. ENSIP-19 for ENS). The client should then obtain the IPNS signature after feeding the encoded `revision` to the IPNS message payload. This signature should then be passed to the gateway among other arguments. The strictly typed formatting for this IPNS signature payload is internally handled by the IPNS protocol and all IPNS service providers. Based on this rough outine, the following section describes the precise interpretation of the metadata common to both IPNS and database storage handlers.
...
function keygen(
  ...
  // Convert hash key to ed25519 keypair [ed25519.priv, ed25519.pub]
  let _ed25519 = ed25519(hashKey)
  // Convert hash key to secp256k1 keypair [secp256k1.priv, secp256k1.pub]
  let _secp256k1 = secp256k1(hashKey)
  // Return both ed25519 and secp256k1 keypairs
  return [_ed25519, _secp256k1]
}
```
...
The final `data:` payload in the off-chain data file must then follow this format,
...
which the CCIP-Read-enabled resolvers should first correctly decode, and then verify signer approval and data signatures, before resolving the data value. The client should construct this `data:` and pass it to the gateway in the `POST` request along with the raw values for indexing.

### `POST` & Protocol-specific Parameters
For any storage other than a blockchain with a wallet extension, the client must call the `gatewayUrl` via a `POST` request. The structure of the `POST` is protocol-specific and left up to individual protocols to handle internally. Besides the `POST` request, `metadataUrl` interface, `username`, `protocol` and `dataType` are four other protocol-specific parameters that we have encountered in the text before. These **five** parameters should be defined by the protocols through a native improvement proposal. For example, `POST` format, `metadataUrl` interface, `username`, `protocol`, and `dataType` for ENS are described in ENSIP-19.
...
function setAddr(
    ...
    bytes memory metadata = abi.encode(
        ...
        metadataUrl // GraphQL endpoint for encoded revision (per ENSIP-16)
    )
