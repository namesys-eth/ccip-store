...
function setValue(
    ...
    require(authorised(node, msg.sender), "NOT_ALLOWED");
    ... // Store data in arbitrary structure
}
```
...
simultaneously in the background without ever prompting the user.
...
Example of a complete `POST` object for updating multiple ENS records for a node is shown below.
...
## Implementation featuring ENS on Database
