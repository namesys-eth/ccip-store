...
A minimal L2 handler only requires the list of `chainId` values and the corresponding `contract` addresses, where the clients must ensure that the calldata is invariant under routing to L2. One example construction of an L2 handler in an L1 contract is given below.
...
In this case, the metadata must contain the bespoke `gatewayUrl`.
...
In the above implementation, the client must follow these steps:

1. Request the user for a secret signature `sigKeygen` to generate a deterministic `dataSigner` keypair,

2. Sign the calldata and produce verifiable data signature `sigData` from generated data signer's private key,

3. Request the user for signature `approval` approving the generated data signer, and

4. Post the calldata to gateway along with signatures `sigData` and `approval`. 

These steps must be completed using the guidelines in the following sub-sections.

#### Generate Data Signer
...
function keygen(
  ...
  let salt = sha256(`${info}:${sha256(password || "")}:${sigKeygen}`);
...
This `keygen()` function requires four variables: `caip10`, `username`, `password` and `sigKeygen`. Their descriptions are given below.

##### `caip10` & `username`
CAIP-10 identifier `caip10` is auto-derived from the connected wallet's checksummed address `wallet` and `chainId`, and `username` is set equal to the derived CAIP-10 identifier.

```js
/* CAIP-10 identifier */
const caip10 = `eip155:${chainId}:${wallet}`
const username = caip10
```
...
##### `sigKeygen`
The `dataSigner` must be derived from the owner or manager keys of a node. Message payload for `sigKeygen` must be formatted as:
...
With this deterministic format for signature message payload, the client must prompt the user for the ethereum signature. Once the user signs the messages, the `keygen()` function can derive the data signer keypair. Since the derived signer can sign multiple instances of off-chain data in the background without prompting the user, it is possible to update data for multiple nodes simultaneously with this method.
 
#### Legitimise Data Signer
...
- post this `approval` and the `dataSigner` along with the calldata in encoded form.

CCIP-Read-enabled contracts can then verify during resolution time that the `approval` attached with the calldata comes from the node's manager or owner and that it approves the expected `dataSigner`. Using this mechanism of delegating signatures to an off-chain signer, no on-chain `dataSigner` needs to be posted. This additional saving comes at the cost of one additional approval signature `approval` that the clients must prompt from the user. This signature must have the following message payload format:
...
#### Sign Calldata
Signature(s) `sigData` accompanying the off-chain calldata must implement the following format in their message payloads:  

```text
Requesting Signature To Update Off-Chain Data\n\nOrigin: ${username}\nData Type: ${dataType}\nData Value: ${dataValue}
```

where `dataType` parameters are protocol-specific.  `text/avatar`, `address/60` etc.

#### Post CCIP-Read Compatible Payload
...
##### `POST`
For any storage other than a blockchain with a wallet extension, the client must call the `gatewayUrl` via a `POST` request. The structure of the `POST` is as follows:
...
Example of a complete `POST` object is shown below.
