...
This `keygen()` function requires three variables: `username`, `password` and `sigKeygen`. Their descriptions are given below.
...
Example of a complete `POST` object for updating multiple ENS records simultaneously is shown below.
...
let signer = keygen(caip10, sigKeygen, password);
// Construct POST body by signing calldata with derived private key
let POST = signData(node, addr, signer.priv);
// POST to gateway
